...
class GiftPairSpec extends AnyFlatSpec {
  ...
  "A GiftPair" should "update a giver/givee" in {
    assert(
      giftPairUpdateGivee("NewBee")(giftPair) == GiftPair("NewBee", "JohLen")
    )
    assert(
      giftPairUpdateGiver("NewBee")(giftPair) == GiftPair("GeoHar", "NewBee")
    )
  }

  it should "convert from JSON" in {
    val gpJson: Either[Error, GiftPair] =
      giftPairJsonStringToGiftPair(jsonString)

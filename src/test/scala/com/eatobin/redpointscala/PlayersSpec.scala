...
class PlayersSpec extends AnyFlatSpec {
  ...
  private val newBee: Player =
    Player("New Bee", Vector(GiftPair("NewBee", "NewBee")))
  ...
  private val rinStaExt: Player = Player(
    "Ringo Starr",
    Vector(GiftPair("JohLen", "GeoHar"), GiftPair("RinSta", "RinSta"))
  )
  private val johLenExt: Player = Player(
    "John Lennon",
    Vector(GiftPair("PauMcc", "RinSta"), GiftPair("JohLen", "JohLen"))
  )
  private val geoHarExt: Player = Player(
    "George Harrison",
    Vector(GiftPair("RinSta", "PauMcc"), GiftPair("GeoHar", "GeoHar"))
  )
  private val pauMccExt: Player = Player(
    "Paul McCartney",
    Vector(GiftPair("GeoHar", "JohLen"), GiftPair("PauMcc", "PauMcc"))
  )
  ...
  private val geoHarGivee: Player =
    Player("George Harrison", Vector(GiftPair("you", "PauMcc")))
  private val geoHarGiver: Player =
    Player("George Harrison", Vector(GiftPair("RinSta", "you")))
  ...
  "Players" should "return an updated player" in {
    assert(
      playersUpdatePlayer("RinSta")(
        Player("New Bee", Vector(GiftPair("NewBee", "NewBee")))
      )(players) == newBeePlayers
    )
  }
  ...
  it should "convert from JSON" in {
    val plrsJson: Either[Error, Map[String, Player]] =
      playersJsonStringToPlayers(jsonString)

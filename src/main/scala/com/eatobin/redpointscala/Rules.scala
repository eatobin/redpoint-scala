...
import com.eatobin.redpointscala.Players.{PlayersTA, playersGetMyGivee}

object Rules {
  def rulesGiveeNotSelf(selfKey: PlayerKeyTA, givee: GiveeTA): Boolean =
    selfKey != givee

  def rulesGiveeNotRecip(selfKey: PlayerKeyTA, givee: GiveeTA, giftYear: GiftYearTA, players: PlayersTA): Boolean = {
    val giveeIsGivingTo = playersGetMyGivee(givee)(players)(giftYear)
    selfKey != giveeIsGivingTo
  }

  def rulesGiveeNotRepeat(selfKey: PlayerKeyTA, givee: GiveeTA, giftYear: GiftYearTA, players: PlayersTA): Boolean = {
